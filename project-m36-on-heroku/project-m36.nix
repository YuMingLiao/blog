...
, ghc-paths, ghci, Glob, gnuplot, hashable, hashable-time
, haskeline, http-api-data, http-conduit, http-types, HUnit, list-t
...
, scotty, semigroups, lib, stm, stm-containers, streamly
...
mkDerivation {
  ...
  libraryHaskellDepends = [
    ...
    ghc-boot ghc-paths ghci Glob gnuplot hashable hashable-time
    haskeline http-api-data list-t monad-parallel MonadRandom mtl
    network old-locale optparse-applicative parallel path-pieces
    QuickCheck quickcheck-instances random-shuffle recursion-schemes
    resourcet rset semigroups stm stm-containers streamly temporary
    text text-manipulate time transformers unix unordered-containers
    uuid vector vector-instances winery zlib megaparsec 
  ];
